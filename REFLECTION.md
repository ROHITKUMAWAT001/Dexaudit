...
**The Reversal:** I reversed this and built a completely "open" audit engine, placing the email gate _only_ at the very end of the flow (after the baseline savings were calculated but before the detailed breakdown was revealed).

**The Rationale:** I reread the assignment prompt which explicitly stated: _"No login required to use the tool. Email is captured after value is shown."_ From an entrepreneurial standpoint, placing an auth-wall upfront creates massive friction, killing the Top-of-Funnel (TOFU) conversion rate. By moving the gate to the end, we trigger the "Sunk Cost Fallacy" and curiosity—users have already invested time selecting their stack and seeing the "Total Savings" number; they are significantly more likely to provide an email to unlock the "How."
