...
interface AuditState {
  ...
  // Actions
  setContext: (data: Partial<Pick<AuditState, "teamSize" | "companyStage" | "primaryUseCase" | "toolUsageFrequency" | "aiMaturity">>) => void
  ...
  persist(
    (set) => ({
      ...
      toggleTool: (toolId) => set((state) => {
        ...
        if (isSelected) {
          const { [toolId]: _, ...remainingDetails } = state.toolDetails
          return {
      ...
      updateToolDetail: (toolId, detail) => set((state) => ({
        toolDetails: {
          ...state.toolDetails,
