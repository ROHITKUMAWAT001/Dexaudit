  ...
  return selectedTools.map((id) => {
    ...
    if (detail.seats > size * 1.2) {
      ...
      if (size < 10) {
        ...
        recommendation = "Switch to Pro Individual plans with centralized billing to save 40% on enterprise overhead."
        confidence = 0.92
      }
    } else if (id === "chatgpt" || id === "claude") {
      // Suggest LLM consolidation
      optimizedSpend = detail.monthlySpend * 0.75
      recommendation = "Redundant LLM detected. Consolidated API access through a single gateway could reduce costs by 25%."
      confidence = 0.85
    } else {
